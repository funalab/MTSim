 ...
 * Last modified: Sun, 30 Jun 2013 02:53:24 +0900
 */
#include "mtsim.h"
...
void draw_graphs(int i, mtGraphics *mtg, double PVecCen[2][3], double MT[][3], double Nuc[3], double DistanceFromPP, double min, double max, double sum, int currentN, double OldVel, double NewVel) {
  ...
  int m;
  for (m=0; m<N; m++) {
    if (m<NN){XDrawLine(d,mtg->pixmap,mtg->gc1,WIN_WIDTH/2+(int)(Scale1*PVecCen[0][0]*pow(10,6)),WIN_HEIGHT/2-(int)(Scale1*PVecCen[0][2]*pow(10,6)),WIN_WIDTH/2+(int)(Scale1*MT[m][0]*pow(10,6)),WIN_HEIGHT/2-(int)(Scale1*MT[m][2]*pow(10,6)));}
  ...
  XDrawPoint(d,mtg->w5,mtg->gc5,(int)(SC3*i),WIN_HEIGHT-(int)(Scale1*pow(10,6)*sum/N));
  ...
  XDrawPoint(d,mtg->w6,mtg->gc6,(int)(SC3*i),WIN_HEIGHT-(int)(2*Scale1*pow(10,6)*L[0]));
