...
/* allocate a float matrix m[nrl..nrh][ncl..nch] that points to the matrix declared in the standad C manner as a[nrow][ncol], where nrow=nrh-nrl+1 and ncol=nch-ncl+1. The routine should be called with the address &a[0][0] as the first argument. */
{
  ...
  m[nrl]=a-ncl;
  for(i=1,j=nrl;i<nrow;i++,j++) m[j]=m[j-1]+ncol;
...
/* free a convert_matrix allocated with convert_matrix() */
{
  free((FREE_ARG) (b+nrl-NR_END));
}
