...
/* void draw_graphs(int i, mtGraphics *mtg, mtGlobal *g, double PVecCen[2][3], double MT[][3], double Nuc[3], double DistanceFromPP, double min, double max, double sum, int currentN, double OldVel, double NewVel, double tempRotationVel[][3]) { */
  Display *d = mtg->d;
  ...
  for (m=0; m<g->N; m++) {
    ...
    /* if (g->pushing_phase[m] == 1) { */
    /*   if (g->CortexMotorNumber[m] == 1) { */
    /*     XFillArc(d,mtg->pixmap,mtg->gc1,WIN_WIDTH/2+(int)(Scale1*MT[m][0]*pow(10,6))-5,WIN_HEIGHT/2-(int)(Scale1*MT[m][2]*pow(10,6))-5,10,10,0,360*64);} */
    /*   else { */
    /*     XDrawArc(d,mtg->pixmap,mtg->gc2,WIN_WIDTH/2+(int)(Scale1*MT[m][0]*pow(10,6))-5,WIN_HEIGHT/2-(int)(Scale1*MT[m][2]*pow(10,6))-5,10,10,0,360*64);} */
    /* } */
  }
  /* XDrawLine(d,mtg->pixmap,mtg->gc2,WIN_WIDTH/2+(int)(Scale1*PVecCen[0][0]*pow(10,6)),WIN_HEIGHT/2-(int)(Scale1*PVecCen[0][2]*pow(10,6)),WIN_WIDTH/2+(int)(Scale1*tempRotationVel[0][0]*pow(10,6)),WIN_HEIGHT/2-(int)(Scale1*tempRotationVel[0][2]*pow(10,6))); */
  /* XDrawLine(d,mtg->pixmap,mtg->gc2,WIN_WIDTH/2+(int)(Scale1*PVecCen[1][0]*pow(10,6)),WIN_HEIGHT/2-(int)(Scale1*PVecCen[1][2]*pow(10,6)),WIN_WIDTH/2+(int)(Scale1*tempRotationVel[1][0]*pow(10,6)),WIN_HEIGHT/2-(int)(Scale1*tempRotationVel[1][2]*pow(10,6))); */
  ...
  XDrawPoint(d,mtg->w3,mtg->gc3,(int)(SC3*i),WIN_HEIGHT-(int)(2*Scale1*sqrt(Nuc[0]*Nuc[0]+Nuc[1]*Nuc[1]+Nuc[2]*Nuc[2])*25/Rad));

  // draw graph in window 4: distance of the pronucleus from the posterior pole of the egg was plotted against time
  XDrawPoint(d,mtg->w4,mtg->gc4,(int)(SC3*i),WIN_HEIGHT-(int)(Scale1*DistanceFromPP*25/Rad));
  //inset
  /**/
  int insetcount=20;
  ...
  {XDrawPoint(d,mtg->w4,mtg->gc4,(int)(14+SC3*i*AMP-insetcount),(int)(114-AMP_Y*Scale1*(DistanceFromPP-LL)*25/Rad));}
  /**/
  /* range of the inset can be alterd, for example... */
  //if ((SC3*i*4>189)&&(SC3*i*4<290)&&(Scale1*(DistanceFromPP-LL)*pow(10,6)*4<110))
  //{XDrawPoint(d,mtg->w4,mtg->gc4,(int)(14+SC3*i*4-190),(int)(124-4*Scale1*(DistanceFromPP-LL)*25/Rad));}
